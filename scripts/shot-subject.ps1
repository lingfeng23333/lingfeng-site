...
$log = Join-Path $dir "shot-subject.txt"
...
try {
  ...
  $out = Join-Path $dir "subject.png"
  if (Test-Path -LiteralPath $out) { Remove-Item -LiteralPath $out -Force }
  $prof = Join-Path $env:TEMP ("edge-subject-" + [guid]::NewGuid().ToString("N"))
  & $edge --headless=new --disable-gpu --hide-scrollbars --no-first-run `
    --disable-extensions --disable-sync `
    "--user-data-dir=$prof" --window-size=1280,900 --virtual-time-budget=12000 `
    "--screenshot=$out" "http://localhost:3100/bangumi/27364" 2>$null
  $waited = 0
  while (-not (Test-Path -LiteralPath $out) -and $waited -lt 30) {
    Start-Sleep -Milliseconds 500
    $waited++
  }
  Add-Content -LiteralPath $log -Value ("subject " + (Test-Path -LiteralPath $out)) -Encoding UTF8
} finally {
