...
$log = Join-Path $dir "shot-home2.txt"
...
try {
  ...
  $targets = @(
    @{ Path = "/"; Name = "home-v2"; Width = 1280; Height = 1000 },
    @{ Path = "/"; Name = "home-v2-mobile"; Width = 390; Height = 1500 }
  )
  foreach ($t in $targets) {
    $out = Join-Path $dir ($t.Name + ".png")
    if (Test-Path -LiteralPath $out) { Remove-Item -LiteralPath $out -Force }
    $prof = Join-Path $env:TEMP ("edge-home2-" + [guid]::NewGuid().ToString("N"))
    & $edge --headless=new --disable-gpu --hide-scrollbars --no-first-run `
      --disable-extensions --disable-sync `
      "--user-data-dir=$prof" --window-size=$($t.Width),$($t.Height) --virtual-time-budget=9000 `
